#!/usr/bin/env python3
"""
This script should be run inside Blender's Scripting tab.
Creates a medically accurate 3D turtle model with shell structure and medical visualization materials.
"""
...
def create_neck_and_head(collection):
    ...
    # Create simple beak by scaling front
    bpy.ops.object.mode_set(mode='EDIT')
    bpy.ops.mesh.select_all(action='SELECT')
    bpy.ops.mesh.extrude_region_move(TRANSFORM_OT_translate={"value":(0, -0.2, -0.05)})
    bpy.ops.transform.resize(value=(0.8, 1.0, 0.9))
    bpy.ops.object.mode_set(mode='OBJECT')
